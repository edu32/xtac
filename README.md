...
`xtac` compiles **xtab** source files into x64 binaries on Windows. On Windows, `xtac` has no external dependencies other than the Windows API and the C runtime library. `xtac` contains (and emits) AVX instructions. `xtac` is multi-threaded and uses upto 1 GB of RAM to compile itself.
